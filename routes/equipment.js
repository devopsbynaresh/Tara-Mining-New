...
router.get("/", async function (req, res) {
  ...
  res.render("equipment/index", {
    ...
    filters: {
      ...
      miningActivity: filters.mineActivityArr
        ? filters.mineActivityArr.$in
        : [],
...
router.get("/search", async function (req, res) {
  ...
  let equipments = await Equipment.find({
    $or: [
      ...
      { oreLocation: { $regex: new RegExp(searchedKey, "i") } },
      { rockHardness: { $regex: new RegExp(searchedKey, "i") } },
  ...
  if (equipments.length > 0) {
    for (const element of equipments) {
      ...
      if (
        element.oreLocation.search(new RegExp(req.query.search, "i")) !== -1
      ) {
        searchedNames.push(element.oreLocation);
      }
      if (
        element.rockHardness.search(new RegExp(req.query.search, "i")) !== -1
      ) {
        searchedNames.push(element.rockHardness);
      }
